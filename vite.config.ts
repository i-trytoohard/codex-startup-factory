      ...
      server.middlewares.use('/api/generate-debate', async (req, res, next) => {
        ...
        try {
          ...
          if (payload.options.length !== 2) {
            throw new Error('Every debate must have exactly 2 people.')
          }

          if (!payload.options.every((option) => option.isDead) || !payload.winningOption.isDead) {
            throw new Error('Every debate must be between deceased people only.')
          }
          ...
          const prompt = [
            ...
            'Both speakers are deceased historical figures.',
